...
class RealPath(object):
    ...
    def validate(self, *_):
        ...
        # FIXME self.generating_vps has no defined order, so last item changes after restarts (esp. for attr/xattr)
        # TODO generate folder on file name collision
        assert len(self.generating_vps) == 1 or self.is_folder, \
            "File name collision: file %s is generated by more than one virtual paths:\n%s" \
            % (self, "\n".join(str(vp) for vp in self.generating_vps))
    ...
    def path(self) -> str:
        try:
            ...
            raise ValueError("File name collision: %s is generated by multiple virtual paths with different paths:\n%s"
                             % (self, "\n".join(str(vp) for vp in self.generating_vps)))
    ...
    def is_folder(self) -> bool:
        try:
            ...
            raise ValueError("File name collision: %s is generated by multiple virtual paths with different folder/file property:\n%s"
                             % (self, "\n".join(str(vp) for vp in self.generating_vps)))
    ...
    def __str__(self):
        return self.path + ("/" if self.is_folder else "") + ("[root]" if self.is_root else "") + \
